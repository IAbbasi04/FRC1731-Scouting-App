...
export function PickListBuilder() {
  ...
  useEffect(() => {
    if (!eventInfo || teams.length === 0) return;
    const saved: SavedPickList = {
      order: teams.map((team) => team.teamNumber),
      tags: Object.fromEntries(teams.map((team) => [String(team.teamNumber), team.tags])),
      customTags,
      manualOrder,
    };
    window.localStorage.setItem(storageKey(eventInfo.key), JSON.stringify(saved));
  }, [customTags, eventInfo, manualOrder, teams]);
  ...
  async function loadEvent(event: FormEvent) {
    ...
    try {
      ...
      const baseTeams: PickListTeam[] = data.teams.map((team) => ({ ...team, tags: [] }));
      let nextTeams = baseTeams;
      ...
      try {
        ...
        if (raw) {
          ...
          for (const teamNumber of saved.order ?? []) {
            ...
            ordered.push({
              ...team,
              tags: Array.isArray(saved.tags?.[String(teamNumber)])
                ? saved.tags[String(teamNumber)].filter((tag): tag is string => typeof tag === "string")
                : [],
            });
          }
        ...
        // A corrupt local draft should not stop a fresh event list from loading.
      }
      ...
      setMessage(
        restored
          ? "Loaded your saved ordering and tags from this device."
          : "Loaded all event teams, initially sorted by OPR.",
      );
    } catch (error) {
  ...
  function toggleTag(teamNumber: number, tag: string) {
    setTeams((current) =>
      current.map((team) => {
        ...
        return {
          ...team,
          tags: team.tags.includes(tag)
            ? team.tags.filter((item) => item !== tag)
            : [...team.tags, tag],
        };
      }),
    );
  ...
  function resetToOpr() {
    ...
    setMessage("Reset to the current OPR order. Existing tags were kept.");
  }
      ...
      <section className="rounded-2xl border border-blue-400/20 bg-[#0d1b2e]/85 p-4 sm:p-6">
        <form onSubmit={loadEvent} className="flex flex-col gap-3 sm:flex-row sm:items-end">
          ...
          <button
            ...
            className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#0b5fff] px-5 py-3 font-semibold text-white hover:bg-blue-500 disabled:opacity-50"
          >
        ...
        <p className="mt-3 text-xs text-slate-500">
          The initial order uses The Blue Alliance OPR. Manual ranking and tags are saved locally per event on this device.
        </p>
        ...
        <>
          <section className="rounded-2xl border border-yellow-300/20 bg-yellow-300/5 p-4 sm:p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {teams.length} teams{eventInfo.city ? ` · ${eventInfo.city}${eventInfo.stateProv ? `, ${eventInfo.stateProv}` : ""}` : ""}
                  {manualOrder ? " · Manual order" : " · OPR order"}
                </p>
              </div>
              <button
                type="button"
                onClick={resetToOpr}
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-yellow-300/30 px-4 py-2.5 text-sm font-semibold text-yellow-100 hover:bg-yellow-300/10"
              >
            ...
            <form onSubmit={addCustomTag} className="mt-4 flex flex-col gap-2 border-t border-yellow-300/10 pt-4 sm:flex-row sm:items-center">
              ...
              <input
                ...
                placeholder="e.g. elite auto"
                className={`${inputClass} sm:max-w-xs`}
              />
              <button type="submit" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-blue-300/20 px-4 py-2 text-sm font-semibold text-slate-200 hover:border-[#ffd84d]/40">
          ...
          <section className="overflow-hidden rounded-2xl border border-blue-400/20 bg-[#0d1b2e]/70">
            <div className="grid grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-2 border-b border-blue-400/10 bg-[#11243d] px-3 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500 sm:grid-cols-[60px_64px_minmax(0,1fr)_120px_110px_auto] sm:px-4">
              ...
              <span>Team</span>
              <span className="hidden sm:block">OPR</span>
              ...
              {teams.map((team, index) => {
                ...
                return (
                  <article
                    ...
                    onDrop={(event) => {
                      event.preventDefault();
                      if (draggingTeam !== null) moveTeam(draggingTeam, team.teamNumber);
                      setDraggingTeam(null);
                    }}
                    className={isDragging ? "bg-[#0b5fff]/10 opacity-60" : "bg-transparent"}
                  >
                    <div className="grid grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-2 px-3 py-3 sm:grid-cols-[60px_64px_minmax(0,1fr)_120px_110px_auto] sm:px-4">
                      ...
                      <div className="hidden items-center sm:flex">
                        <button
                          ...
                          draggable
                          onDragStart={(event) => {
                            setDraggingTeam(team.teamNumber);
                            event.dataTransfer.effectAllowed = "move";
                            event.dataTransfer.setData("text/plain", String(team.teamNumber));
                          }}
                          onDragEnd={() => setDraggingTeam(null)}
                          className="cursor-grab rounded-lg p-2 text-slate-500 hover:bg-[#0b5fff]/15 hover:text-slate-200 active:cursor-grabbing"
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-baseline gap-x-2">
                          ...
                          <span className="truncate text-sm text-slate-300">{team.nickname}</span>
                        </div>
                        <div className="mt-1 flex flex-wrap gap-1.5 sm:hidden">
                          <span className="text-xs text-slate-500">OPR {team.opr === null ? "—" : team.opr.toFixed(1)}</span>
                          <span className="text-xs text-slate-600">· TBA rank {team.rank ?? "—"}</span>
                        </div>
                        {team.tags.length ? (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            {team.tags.map((tag) => (
                              <span key={tag} className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold ${tagClass(tag)}`}>{tag}</span>
                            ))}
                          </div>
                        ) : null}
                      </div>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        <button
                          type="button"
                          onClick={() => moveBy(team.teamNumber, -1)}
                          disabled={index === 0}
                          className="rounded-lg border border-blue-300/15 p-2 text-slate-400 hover:text-white disabled:opacity-25"
                          aria-label={`Move Team ${team.teamNumber} up`}
                        >
                          <ChevronUp size={17} />
                        </button>
                        <button
                          ...
                          className="rounded-lg border border-blue-300/15 p-2 text-slate-400 hover:text-white disabled:opacity-25"
                          aria-label={`Move Team ${team.teamNumber} down`}
                        >
                          <ChevronDown size={17} />
                        </button>
                        <button
                          type="button"
                          onClick={() => setExpandedTeam(expanded ? null : team.teamNumber)}
                          className={`rounded-lg border px-2.5 py-2 text-xs font-semibold ${expanded ? "border-[#ffd84d]/40 bg-[#ffd84d]/10 text-[#ffd84d]" : "border-blue-300/15 text-slate-300 hover:border-[#ffd84d]/35"}`}
                        >
                          Tags
                        </button>
                      </div>
                      ...
                      <div className="border-t border-blue-400/10 bg-[#07111f]/55 px-3 py-3 sm:px-4">
                        <div className="flex flex-wrap gap-2">
                          {availableTags.map((tag) => {
                            const selected = team.tags.includes(tag);
                            return (
                              <button
                                key={tag}
                                type="button"
                                onClick={() => toggleTag(team.teamNumber, tag)}
                                className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${selected ? tagClass(tag) : "border-blue-300/15 bg-[#07111f] text-slate-400 hover:border-blue-300/30 hover:text-slate-200"}`}
                              >
                                {selected ? "✓ " : "+ "}{tag}
                              </button>
                            );
          ...
          <p className="px-1 text-xs text-slate-600">
            Drag the grip handle on desktop, or use the arrow buttons on any device. Changes save automatically on this device.
          </p>
        </>
      ) : null}
    </div>
