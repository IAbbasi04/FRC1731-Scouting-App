...
import { CalendarClock, RefreshCw, UserRoundCheck } from "lucide-react";
...
export function ScoutingAssignmentPanel() {
  ...
  function loadAssignment(match: TbaMatch) {
    ...
    setMessage(`Loaded Q${match.match_number} · Team ${team} · ${stationInfo.label}.`);
    ...
    <section className="rounded-2xl border border-blue-400/20 bg-[#0d1b2e]/85 p-5 sm:p-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-slate-500">Choose the scout and physical station once. The event schedule then fills the correct match, team, alliance, and scout into the scouting form.</p>
        </div>
        <div className="rounded-xl border border-yellow-300/15 bg-yellow-300/5 px-3 py-2 text-xs text-slate-400">Schedule is cached locally after a successful load.</div>
      </div>
      ...
      <form onSubmit={loadSchedule} className="mt-5 grid gap-3 md:grid-cols-[1fr_1fr_180px_auto] md:items-end">
        ...
        <label className="space-y-2 text-sm"><span className="font-medium text-slate-300">Station</span><select value={station} onChange={(event) => setStation(event.target.value as Station)} className={inputClass}>{stations.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
        ...
        <div className="mt-5">
          ...
          <div className="max-h-[360px] space-y-2 overflow-y-auto pr-1">
            {qualificationMatches.map((match) => {
              ...
              return (
                <button key={match.key} type="button" onClick={() => loadAssignment(match)} disabled={!team} className={`flex w-full items-center justify-between gap-4 rounded-xl border px-4 py-3 text-left transition ${completed ? "border-blue-300/10 bg-[#07111f]/35" : "border-blue-300/20 bg-[#07111f]/70 hover:border-[#ffd84d]/45 hover:bg-[#0b5fff]/10"} disabled:opacity-40`}>
                  ...
                  <span className="rounded-lg border border-blue-300/15 px-3 py-1.5 text-xs font-semibold text-slate-300">Load</span>
                </button>
              );
            })}
          </div>
