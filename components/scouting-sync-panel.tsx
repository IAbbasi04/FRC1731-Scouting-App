...
export function ScoutingSyncPanel() {
  ...
  async function syncNow() {
    ...
    if (!configured) {
      setMessage("Cloud sync is not configured yet. Initialize Convex, then add NEXT_PUBLIC_CONVEX_URL.");
      return;
    }
    if (pendingLatest.length === 0) {
      setEntries(latest);
      setMessage("Everything in the current queue is already synced.");
      return;
    }
    ...
    try {
      ...
      setMessage(
        failedIds.length === 0
          ? `Synced ${syncedIds.length} entr${syncedIds.length === 1 ? "y" : "ies"}.`
          : `Synced ${syncedIds.length}; ${failedIds.length} remain queued for retry.`,
      );
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Cloud sync failed. Entries remain saved locally.");
    } finally {
      setSyncing(false);
    }
  }
  ...
  return (
    <section className="rounded-2xl border border-blue-400/20 bg-[#0d1b2e]/80 p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            {configured ? <Cloud size={19} className="text-[#ffd84d]" /> : <CloudOff size={19} className="text-slate-500" />}
            <h2 className="font-semibold text-white">Scouting cloud sync</h2>
          </div>
          <p className="mt-1 text-sm text-slate-500">
            Local storage remains the source of safety. Cloud sync copies entries to the shared team database when connectivity is available.
          </p>
        </div>
      ...
      {!configured ? <p className="mt-3 text-xs text-slate-600">Setup required once per project: run `npx convex dev` locally and keep the generated NEXT_PUBLIC_CONVEX_URL in `.env.local`.</p> : null}
    </section>
  );
}
